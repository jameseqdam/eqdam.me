...
const ContactReveal = ({ channel, variant = 'card', className }: ContactRevealProps) => {
  ...
  /** Only keyboard reveals should move focus; a mouse reveal must not. */
  const heldByKeyboardRef = useRef(false);
  ...
  // Revealing swaps the pressed card out for the action buttons, which would
  // strand keyboard focus on a removed node.
  useEffect(() => {
    if (revealed && heldByKeyboardRef.current) copyButtonRef.current?.focus();
  }, [revealed]);
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLButtonElement>) => {
    ...
    heldByKeyboardRef.current = true;
    startHold();
  };
  ...
  // Glassmorphic shell built from the site's own tokens; shared so the locked
  // and revealed states are the same object rather than two lookalikes.
  ...
  const iconBadge = (
    <span
      className={cn(
        'relative flex flex-shrink-0 items-center justify-center rounded-lg bg-primary/10',
  ...
  const valueBlock = (
    <span className="relative min-w-0 flex-1">
      {isCard && <span className="block text-sm font-medium sm:text-base">{config.label}</span>}
      <span
        className={cn(
          'block truncate text-muted-foreground transition-[filter,opacity,color] duration-500 ease-out',
          isCard ? 'text-sm sm:text-base' : 'text-sm',
          revealed ? 'text-foreground opacity-100 blur-0' : 'select-none opacity-70 blur-[8px]',
        )}
  ...
  // Locked: the entire card is the press target, not just the lock icon.
  if (!revealed || !href) {
    return (
      <button
        type="button"
        ...
        className={cn(
          shellClasses,
          // pan-y keeps the page scrollable when a touch starts on the card;
          // the browser then fires pointercancel, which aborts the hold.
          'w-full touch-pan-y select-none overflow-hidden text-left hover:border-primary/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring',
        )}
      >
        {/* Progress sweeps the full width, tying the fill to the whole target. */}
        <span
          aria-hidden="true"
        ...
        <span className="relative flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full text-muted-foreground transition-colors group-hover:text-primary">
          <svg className="absolute inset-0 h-full w-full -rotate-90" viewBox="0 0 32 32">
            ...
            <circle
              ...
              style={{
                filter: progress > 0 ? 'drop-shadow(0 0 4px hsl(var(--primary) / 0.6))' : undefined,
              }}
            />
          </svg>
